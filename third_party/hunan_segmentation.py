...
class Segmentation:
    ...
    def __init__(self, imgPath: str, mask: np.ndarray, w=15, angle=15, k1=.67, k2=.33, t=.31, block=11, L=7, eps=.5, t1=90,
                 e1=7, e2=5, a=9, p1=.3, p2=.14):
    ...
    def basicLineDetector(self):
        ...
        def R_b(y, x):
            # creating window
            yIndex, xIndex = np.ogrid[y - int((self.w - 1) / 2):y + int((self.w + 1) / 2),
                             x - int((self.w - 1) / 2):x + int((self.w + 1) / 2)]
    ...
    def tracing(self):
        ...
        def circleMask():
            ...
            mask[skimage.draw.disk((center, center), self.L + self.eps)] = 1
            mask[skimage.draw.disk((center, center), self.L - self.eps)] = 0
        ...
        def psi(x, x1, y, y1, img=img):
            ...
            for k in range(1, self.L + 1):
                alpha += img[int(np.rint((k * y + (self.L - k) * y1) / self.L)), int(
                    np.rint((k * x + (self.L - k) * x1) / self.L))]
            return alpha
