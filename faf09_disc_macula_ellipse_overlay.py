...
def write_overlay(image_size, disc_center: tuple, macula_center: tuple, outfile_path: Path):
    # Create a new SVG drawing with specified width and height
    dwg = svgwrite.Drawing(str(outfile_path), size=image_size)
    dist = disc_macula_distance(disc_center, macula_center)
    ...
    dwg.add(dwg.circle(center=macula_center, r=int(round(dist*GEOMETRY["fovea_radius"])), fill='green'))

    # the inner ellipse
    radii = tuple(i*dist for i in GEOMETRY["ellipse_radii"])
    (x, y) = macula_center
    angle = macula_disc_angle(macula_center, disc_center)
    extra_args = {"stroke": "red", "fill": "none", "stroke-width": int(round(dist/12)),
                  "transform": f"rotate({angle} {x} {y})"}
    dwg.add(dwg.ellipse(center=macula_center, r=radii,  **extra_args))
...
def main():
    ...
    for faf_img_data in FafImage.select().where(FafImage.usable == True):
        ...
        macula_center = (faf_img_dict['macula_x'], faf_img_dict['macula_y'])

        svg_filepath  = construct_workfile_path(WORK_DIR, faf_img_dict['image_path'],
                                                faf_img_dict['case_id']['alias'], 'overlay', 'svg')

        write_overlay(image_size, disc_center, macula_center, svg_filepath)
